...
type BundlePricing = {
  byProduct: Map<string, BundleItemMetadata[]>;
  discountMinor: number;
};
...
function parseItems(value: unknown): RequestedItem[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > 30) {
    throw new ApiError(
      ...
      { items: "Add between 1 and 30 items." },
    );
  }
  ...
  value.forEach((raw, index) => {
    ...
    const quantity = integerField(item.quantity, `items.${index}.quantity`, {
      min: 1,
      max: 20,
    });
...
async function priceItems(
  ...
  return requested.map((item, index) => {
    const row = results[index]?.results?.[0] as
      Omit<PricedItem, "quantity"> | undefined;
    if (!row)
      throw new ApiError(
        422,
        "PRODUCT_UNAVAILABLE",
...
async function validateBundleMetadata(
  db: D1Database,
  value: unknown,
  requested: PricedItem[],
): Promise<BundlePricing> {
  const byProduct = new Map<string, BundleItemMetadata[]>();
  if (value === undefined || value === null)
    return { byProduct, discountMinor: 0 };
  if (!Array.isArray(value) || value.length > 10) {
    throw new ApiError(
      422,
      "INVALID_BUNDLE_METADATA",
      "Bundle selections must be a valid list.",
    );
  }
  const cartQuantities = new Map(
    requested.map((item) => [item.productId, item.quantity]),
  );
  const unitPrices = new Map(
    requested.map((item) => [item.productId, item.unitPriceMinor]),
  );
  ...
  for (let bundleIndex = 0; bundleIndex < value.length; bundleIndex += 1) {
    ...
    if (
      ...
      throw new ApiError(
        422,
        "INVALID_BUNDLE_METADATA",
        "Bundle selections must be valid.",
      );
    }
    const bundleInput = rawBundle as Record<string, unknown>;
    const bundleId = requiredString(
      bundleInput.bundleId,
      `bundleMetadata.${bundleIndex}.bundleId`,
      { min: 1, max: 100 },
    );
    const bundle = await db
      .prepare(
        `SELECT id, name, pricing_mode AS pricingMode,
      price_value_minor AS priceValueMinor FROM bundles WHERE id = ? AND status = 'ACTIVE'`,
      )
      .bind(bundleId)
      .first<{
        id: string;
    ...
    if (!bundle)
      throw new ApiError(
        422,
        "BUNDLE_UNAVAILABLE",
        "A selected set is no longer available.",
      );
    if (!Array.isArray(bundleInput.selections)) {
      throw new ApiError(
        422,
        "INVALID_BUNDLE_METADATA",
        "Each set must include its step selections.",
      );
    }
    const configuredSteps = await db
      .prepare(
        `SELECT id, name, min_selections AS minSelections,
    ...
    if (selections.length !== configuredSteps.results.length) {
      throw new ApiError(
        422,
        "INCOMPLETE_BUNDLE",
        "Choose a product for every set step.",
      );
    }
    const instanceId = randomId("bundle_instance");
    let bundleSubtotalMinor = 0;
    for (const configuredStep of configuredSteps.results) {
      const rawSelection = selections.find((selection) =>
        Boolean(
          selection &&
      ...
      if (
        ...
        throw new ApiError(
          422,
          "INCOMPLETE_BUNDLE",
          `Choose a product for ${configuredStep.name}.`,
        );
      }
      const productIdsRaw = (rawSelection as Record<string, unknown>)
        .productIds;
      if (!Array.isArray(productIdsRaw))
        throw new ApiError(
          422,
          "INVALID_BUNDLE_METADATA",
          "Step productIds must be a list.",
        );
      const productIds = productIdsRaw.map((productId, productIndex) =>
        requiredString(
          productId,
      ...
      if (
        productIds.length < configuredStep.minSelections ||
        productIds.length > configuredStep.maxSelections
      ) {
        throw new ApiError(
          422,
          "INVALID_BUNDLE_SELECTION_COUNT",
          `Choose the required number of products for ${configuredStep.name}.`,
        );
      }
      for (const productId of productIds) {
        ...
        if (!cartQuantity)
          throw new ApiError(
            422,
            "BUNDLE_ITEM_MISSING",
            `${productId} is not in the order items.`,
          );
        const allowed = await db
          .prepare(
            `SELECT 1 AS found FROM bundle_step_options o
          JOIN product_variants v ON v.id = o.product_variant_id
          WHERE o.step_id = ? AND v.product_id = ? AND o.enabled = 1 LIMIT 1`,
          )
          .bind(configuredStep.id, productId)
          .first<{ found: number }>();
        if (!allowed)
          throw new ApiError(
            422,
            "BUNDLE_OPTION_UNAVAILABLE",
            `${productId} is not available for ${configuredStep.name}.`,
          );
        const nextAllocated = (allocatedQuantities.get(productId) ?? 0) + 1;
        if (nextAllocated > cartQuantity) {
          throw new ApiError(
            422,
            "BUNDLE_ITEM_QUANTITY_MISMATCH",
            `${productId} needs another unit in the order items.`,
          );
        }
        allocatedQuantities.set(productId, nextAllocated);
        bundleSubtotalMinor += unitPrices.get(productId) ?? 0;
    ...
    if (bundle.pricingMode === "FIXED_DISCOUNT") {
      discountMinor += Math.min(
        bundleSubtotalMinor,
        Math.max(0, bundle.priceValueMinor ?? 0),
      );
    } else if (bundle.pricingMode === "PERCENT_DISCOUNT") {
      const basisPoints = Math.min(
        10_000,
        Math.max(0, bundle.priceValueMinor ?? 0),
      );
      discountMinor += Math.floor((bundleSubtotalMinor * basisPoints) / 10_000);
    }
  }
  return { byProduct, discountMinor };
}
...
async function calculatePromotion(
  ...
  if (subtotalMinor < promotion.minSubtotalMinor) {
    throw new ApiError(
      ...
      `Spend RM${(promotion.minSubtotalMinor / 100).toFixed(2)} to use this offer.`,
    );
  }
  ...
  if (
    ...
    Number(totalUses?.count ?? 0) >= promotion.usageLimit
  ) {
    throw new ApiError(
      422,
      "PROMO_LIMIT_REACHED",
      "That offer has reached its usage limit.",
    );
  }
  if (userId && promotion.perCustomerLimit !== null) {
    const customerUses = await db
      .prepare(
        "SELECT COUNT(*) AS count FROM promotion_redemptions WHERE promotion_id = ? AND user_id = ?",
      )
      .bind(promotion.id, userId)
      .first<{ count: number }>();
  ...
  if (promotion.discountType === "PERCENTAGE") {
    discountMinor = Math.floor(
      (subtotalMinor * promotion.percentBasisPoints) / 10_000,
    );
...
async function shippingSettings(db: D1Database) {
  const settings = await db
    .prepare(
      `SELECT shipping_fee_minor AS shippingFeeMinor,
...
export async function handlePromoValidation(
  ...
  await consumeRateLimit(db, request, {
    bucket: "promo-validation",
    limit: 30,
    windowSeconds: 5 * 60,
    code: "PROMO_RATE_LIMITED",
    message: "Too many offer checks. Please wait and try again.",
  });
  ...
  const standardShipping =
    subtotalMinor >= settings.thresholdMinor ? 0 : settings.shippingFeeMinor;
  return ok({
    valid: true,
    code: promotion.code,
...
function parseShippingAddress(value: unknown): ShippingAddress {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ApiError(422, "VALIDATION_ERROR", "Enter a shipping address.", {
      shippingAddress: "Required.",
    });
  }
  const body = value as Record<string, unknown>;
  const postcode = requiredString(body.postcode, "shippingAddress.postcode", {
    min: 5,
    max: 5,
  });
  if (!/^\d{5}$/u.test(postcode)) {
    throw new ApiError(
      ...
      { "shippingAddress.postcode": "Enter five digits." },
    );
  }
  return {
    ...
    phone: normalizePhone(
      requiredString(body.phone, "shippingAddress.phone", { min: 7, max: 30 }),
    ),
    line1: requiredString(body.line1, "shippingAddress.line1", {
      min: 3,
      max: 180,
    }),
    line2: optionalString(body.line2, "shippingAddress.line2", 180),
    city: requiredString(body.city, "shippingAddress.city", {
      min: 2,
      max: 100,
    }),
    state: requiredString(body.state, "shippingAddress.state", {
      min: 2,
      max: 100,
    }),
    postcode,
    country: requiredString(
      body.country ?? "Malaysia",
...
export async function handleCreateOrder(
  ...
  const bundlePricing = await validateBundleMetadata(
    db,
    body.bundleMetadata,
    items,
  );
  ...
  if (unavailable)
    throw new ApiError(
      409,
      "OUT_OF_STOCK",
      `${unavailable.name} does not have enough stock.`,
    );
  const shippingAddress = parseShippingAddress(body.shippingAddress);
  const paymentMethod = requiredString(body.paymentMethod, "paymentMethod", {
    min: 1,
    max: 50,
  });
  if (paymentMethod !== "manual_confirmation") {
    throw new ApiError(
      ...
      "Only manual payment confirmation is currently available.",
    );
  }
  const profile = await db
    .prepare(
      `SELECT u.email, p.full_name AS fullName, p.phone_e164 AS phone
    FROM users u JOIN customer_profiles p ON p.user_id = u.id WHERE u.id = ?`,
    )
    .bind(session.user.id)
    .first<{ email: string; fullName: string; phone: string | null }>();
  if (!profile)
    throw new ApiError(
      404,
      "PROFILE_NOT_FOUND",
  ...
  const promotion = await calculatePromotion(
    db,
    optionalString(body.promoCode, "promoCode", 50),
    subtotalMinor,
    session.user.id,
  );
  ...
  const statements: D1PreparedStatement[] = [
    db
      ...
      .bind(
        ...
        orderNumber,
        session.user.id,
        shippingAddress.recipientName || profile.fullName,
        profile.email,
    ...
    db
      ...
      .bind(
        ...
        shippingAddress.country === "Malaysia" ? "MY" : shippingAddress.country,
      ),
    db
      .prepare(
      ...
      statements.push(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          ...
          .bind(
            ...
            bundle?.bundleInstanceId ?? null,
            bundle?.bundleName ?? null,
            bundle?.stepName ?? null,
          ),
      );
  ...
  try {
    ...
    if (message.includes("idempotency_keys")) {
      const raced = await db
        .prepare(
          "SELECT request_hash AS requestHash, response_json AS responseJson FROM idempotency_keys WHERE key_hash = ?",
        )
        .bind(keyHash)
        .first<{ requestHash: string; responseJson: string | null }>();
      if (raced?.requestHash === requestHash && raced.responseJson)
        return ok(JSON.parse(raced.responseJson));
    }
    throw error;
  }
...
export async function handleNewsletter(
  ...
  await db
    ...
    .bind(randomId("subscriber"), email, email)
    .run();
  return ok({ subscribed: true }, 201);
}

export async function handleCreateEnquiry(
  request: Request,
  db: D1Database,
): Promise<Response> {
  assertSameOrigin(request);
  ...
  await db.batch([
    db
      ...
      .bind(
        ...
        phoneValue ? normalizePhone(phoneValue) : null,
        subject,
      ),
    db
      .prepare(
