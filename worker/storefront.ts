...
export async function loadProducts(db: D1Database, includeInactive = false) {
  const products = await allRows<ProductRow>(
    db.prepare(`SELECT
  ...
  return products.map((product) => ({
    ...
    benefits: benefits
      .filter((benefit) => benefit.productId === product.id)
...
export async function loadBundles(db: D1Database, includeInactive = false) {
  const bundles = await allRows<{
    ...
    pricing_mode AS pricingMode, price_value_minor AS priceValueMinor
  ...
  return bundles.map((bundle) => ({
    ...
    discountType:
      bundle.pricingMode === "FIXED_DISCOUNT"
        ? "fixed"
        : bundle.pricingMode === "PERCENT_DISCOUNT"
          ? "percentage"
          : "none",
    discountValue:
      bundle.priceValueMinor === null ? 0 : bundle.priceValueMinor / 100,
    sortOrder: bundle.sortOrder,
    steps: steps
      .filter((step) => step.bundleId === bundle.id)
      .map((step) => ({
        id: step.id,
        label: step.label,
        description: step.description,
        productIds: options
          .filter((option) => option.stepId === step.id && option.enabled)
          .map((option) => option.productId),
        defaultProductId:
          options.find(
            (option) =>
              option.stepId === step.id && option.enabled && option.isDefault,
          )?.productId ?? null,
...
export async function storefrontPayload(db: D1Database) {
  const [settingsRow, socials, products, slides, gallery, bundles, sections] =
    await Promise.all([
      db
        .prepare(
          `SELECT brand_name AS storeName, support_email AS supportEmail,
      ...
      FROM store_settings WHERE id = 'default'`,
        ...
        .first<{
          ...
          shippingThresholdMinor: number;
        }>(),
      allRows<{ platform: string; handle: string | null; url: string }>(
        db.prepare(
          "SELECT platform, handle, url FROM social_links WHERE enabled = 1 ORDER BY sort_order",
        ),
      ),
      loadProducts(db),
      allRows<{
  ...
  if (!settingsRow)
    throw new ApiError(
      ...
      "Store settings are not available yet.",
    );
