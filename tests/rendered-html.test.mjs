...
test("server-renders the current 3R&Co storefront", async () => {
  ...
  assert.match(html, /<title>3R&amp;Co Malaysia \| Moringa Body Oil &amp; Body Care<\/title>/i);
  assert.match(html, /rel="canonical" href="https:\/\/3rnco\.com\.my\/?"/i);
  assert.match(html, /property="og:image" content="https:\/\/3rnco\.com\.my\/og-3rnco-moringa-v2\.png"/i);
...
test("includes persistent production commerce and clean seed contracts", async () => {
  const [page, account, admin, bundle, api, layout, css, packageJson, phpSeeder, mysqlSchema] = await Promise.all([
    ...
    readFile(new URL("../app/components/BundleBuilder.tsx", import.meta.url), "utf8"),
  ...
  assert.doesNotMatch(page, /data\.(?:products|slides|gallery|bundles)\?\.length/);
  ...
  for (const source of [page, account, admin, bundle, phpSeeder]) {
    assert.doesNotMatch(source, /ROOTED10|WELCOME15|TRAVELDUO|CARE15|HELLOSHIP|SLOW20|example\.test|Place demo order|commerce simulation/i);
  }
