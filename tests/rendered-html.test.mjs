...
test("server-renders the current 3R&Co storefront", async () => {
  ...
  assert.match(html, /Care, passed gently on/);
  assert.match(html, /Small gestures\./);
  assert.match(html, /\/images\/campaign\/care-passed-gently-v8\.webp/);
...
test("includes persistent production commerce and clean seed contracts", async () => {
  ...
  assert.match(
    productionCss,
    /height:\s*min\(900px,\s*calc\(100dvh\s*-\s*48px\)\)/,
  );
  assert.match(
    productionCss,
    /\.referral-report \.referral-commission-table > div/,
  );
  assert.doesNotMatch(
    productionCss,
